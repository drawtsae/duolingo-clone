...
const ConsolidationUnit1 = ({navigation}) => {
    ...
    const update = async() =>{
        //update vocabulary
        if (userData?.GrammarUnit1 == "0%"){
            await UpdateTienDo(auth.session?.user.id, value);
        };
        // update unit 1
        if (userData2?.TiendoUnit1 == "75%" ){
            await UpdateTienDo2(auth.session?.user.id, '100%');
        }
    }
           ...
           <View style={{
               ...
               >
                   ...
                   <View style={{
                       ...
                       <TouchableOpacity
                           onPress={()=> {navigation.navigate("GameUnit1Memory"), update()}}
                           style={{
                                marginTop:'10%',
                               backgroundColor: COLORS.accent,
                               padding: 20, width: '100%', borderRadius: 20
...
export default ConsolidationUnit1