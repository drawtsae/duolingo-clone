...
//Main
const TracnghiemFirstVocabularyUnit1 = ({navigation}) => {
    ...
    const update = async() =>{
        //update vocabulary
        if (userData?.VocabularyUnit1 == "0%"){
            await UpdateTienDo(auth.session?.user.id, value);
        };
        // update unit 1
        if (userData2?.TiendoUnit1 == "0%"){
            await UpdateTienDo2(auth.session?.user.id, '12.5%');
        }
        if (userScore?.ScoreUnit1 == 0){
            await UpdateScore(auth.session?.user.id, score);
        }
    }
    const allQuestions = data1;
           ...
           <View style={{
               ...
               >
                   ...
                   <View style={{
                       ...
                       <TouchableOpacity
                           onPress={()=> {navigation.navigate("TracnghiemSecondVocabularyUnit1"),update()}}
                           style={{
                                marginTop:'10%',
                               backgroundColor: COLORS.accent,
                               padding: 20, width: '100%', borderRadius: 20
